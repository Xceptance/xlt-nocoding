package com.xceptance.xlt.nocoding.scriptItem.store;
...
public class StoreTest
{
    ...
    {
        final ScriptItem store = new Store("test", "text");
    ...
    {
        ScriptItem store = new Store("test", "text");
        Assert.assertNull(context.getVariables().get("test"));
        store.execute(context);
        Assert.assertEquals("text", context.getVariables().get("test"));
    ...
    {
        ScriptItem store = new Store("var_1", "val_1");
