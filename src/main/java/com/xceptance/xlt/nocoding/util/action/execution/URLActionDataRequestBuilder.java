...
public class URLActionDataRequestBuilder
{
    ...
    public WebRequest buildRequest(final URLActionData action)
    {
        ...
        {
            throw new IllegalArgumentException("Failed to create WebRequest for action: " + action.getName() + " Reason: " + e.getMessage(),
                                               e);
        }
        ...
        throws UnsupportedEncodingException, MalformedURLException
    {
        ...
        {
            List<NameValuePair> resultParameters = new ArrayList<>(parameters);
            if (!encodeParameters)
            {
    ...
    {
        final List<NameValuePair> encodedParameters = new ArrayList<>();
