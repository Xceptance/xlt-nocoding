...
/**
 * <p>
 * Data container which holds all the necessary information to grab and store information out of a Http response in
 * order to provide access to them via the {@link #interpreter}.
 * </p>
 *
 * @author matthias
 */
public class URLActionDataStore
{
    ...
    public final static Set<String> PERMITTEDSUBSELECTIONMODE = new HashSet<>();
     ...
     *            {@link #interpreter}
     */
    public URLActionDataStore(final String name, final String selectionMode, final String selectionContent,
    ...
    public URLActionDataStore(final String name, final String selectionMode, final String selectionContent, final String subSelectionMode,
        final String subSelectionValue, final ParameterInterpreter interpreter)
    {
        XltLogger.runTimeLogger.debug("Creating new Instance");
        setName(name);
        setSelectionMode(selectionMode);
    ...
    public void outlineRaw()
    {
        ...
        System.err.println("\t\t\t" + selectionMode + " : " + selectionContent);
    }
    ...
    public void setSelectionContent(final String selectionContent)
    {
        ...
        XltLogger.runTimeLogger.debug(MessageFormat.format("Set 'Selection Content': \"{0}\"", selectionContent));
    }
    ...
    public void setSubSelectionMode(final String subSelectionMode)
    {
        ...
        XltLogger.runTimeLogger.debug(MessageFormat.format("Set 'Sub-Selection Mode': \"{0}\"", subSelectionMode));
    }
    ...
    public void setSubSelectionValue(final String subSelectionValue)
    {
        ...
        XltLogger.runTimeLogger.debug(MessageFormat.format("Set 'Sub-Selection Value': \"{0}\"", subSelectionValue));
    }
    ...
    public void setSelectionMode(final String selectionMode)
    {
        ...
        XltLogger.runTimeLogger.debug(MessageFormat.format("Set 'Selection Mode': \"{0}\"", selectionMode));
    }
    ...
    /**
     * @return {@link #subSelectionMode }, after its dynamic interpretation via the {@link #interpreter}.
     */
    public String getSubSelectionMode()
    {
        ...
        {
            dynamicSubSelectionMode = interpreter.processDynamicData(subSelectionMode);
            if (!isPermittedSubSelectionMode(dynamicSubSelectionMode))
            {
                throw new IllegalArgumentException(getIllegalValueForTagMessage(dynamicSubSelectionMode, "Sub-Selection Mode"));
            }
        }
    ...
    {
        return interpreter.processDynamicData(subSelectionValue);
    }
    ...
    {
        final String message = MessageFormat.format("Store: \"{0}\", tag \"{1}\"  cannot be NULL", name, tag);
        return message;
    }
    ...
    {
        final String message = MessageFormat.format("Store: \"{0}\", Illegal value: \"{1}\" for tag \"{2}\"", name, value, tag);
        return message;
    }
}
