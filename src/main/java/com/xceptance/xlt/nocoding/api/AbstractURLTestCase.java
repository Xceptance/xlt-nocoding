...
import com.xceptance.xlt.nocoding.util.action.data.URLActionDataListFacade;
...
import com.xceptance.xlt.nocoding.util.action.execution.URLActionDataExecutionableFactory;
import com.xceptance.xlt.nocoding.util.action.execution.URLActionDataExecutionbleFactoryBuilder;
import com.xceptance.xlt.nocoding.util.action.execution.URLActionDataRequestBuilder;
import com.xceptance.xlt.nocoding.util.action.validation.URLActionDataResponseHandler;
import com.xceptance.xlt.nocoding.util.action.validation.URLActionDataStoreResponseHandler;
import com.xceptance.xlt.nocoding.util.action.validation.URLActionDataValidationResponseHandler;
...
public class AbstractURLTestCase extends AbstractTestCase
{
    ...
    {
        filePath = dataDirectory + File.separatorChar + filePath;
    }
    ...
    {
        urlActionListFacade = new URLActionDataListFacade(filePath, interpreter);
    }

    private void setupURLActionExecutableFactory()
    {
        final URLActionDataExecutionbleFactoryBuilder factoryBuilder = new URLActionDataExecutionbleFactoryBuilder(propertiesAdmin, mode);
        executionableFactory = factoryBuilder.buildFactory();
    }

    private void setupURLActionRequestBuilder()
    {
        requestBuilder = new URLActionDataRequestBuilder();
    }
    ...
    private void setupURLActionResponseHandler()
    {
        ...
        responseHandler = new URLActionDataResponseHandler(storeHandler, validationHandler);
    }
